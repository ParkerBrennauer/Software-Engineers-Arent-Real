...
def get_current_owner(current_user: dict=Depends(get_current_user)) -> dict:
    if current_user.get('role') != 'owner':
        raise HTTPException(status_code=403, detail='Only restaurant owners can perform this action')
    return current_user
...
@router.post('/')
async def create_discount(payload: DiscountCreate, current_user: dict=Depends(get_current_owner)):
    if current_user.get('restaurant_id') != payload.restaurant_id:
        raise HTTPException(status_code=403, detail='You can only manage discounts for your own restaurant')
    result = await DiscountServices.createDiscount(payload.discount_rate, payload.discount_code, payload.restaurant_id)
    if result == 'code is invalid':
        raise HTTPException(status_code=400, detail=result)
    return {'message': result}

@router.delete('/{discount_code}')
async def remove_discount(discount_code: str, current_user: dict=Depends(get_current_owner)):
