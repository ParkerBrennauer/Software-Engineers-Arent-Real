...
class RestaurantService:
    ...
    async def get_restaurants_search_advance(query: str, filters: list, sort: str) -> list:
        ...
        for restaurant in restaurants.values():
            ...
            if query in str(restaurant['restaurant_id']):
                for search_filter in filters:
                    if search_filter in attributes:
                        matches_filter = True
                        break
                if matches_filter:
                    results.append(restaurant)
        if sort == 'AlphabetAsc':
            ...
            results.sort(key=lambda r: r.get('restaurant_id'), reverse=True)
        elif sort == 'RatingAsc':
            results.sort(key=lambda r: float(r.get('avg_ratings', 0)))
        elif sort == 'RatingDesc':
            results.sort(key=lambda r: float(r.get('avg_ratings', 0)), reverse=True)
        return results
    ...
    async def create_restaurant(restaurant_in: RestaurantCreate) -> dict:
        ...
        restaurant_data['menu'] = [item.model_dump() for item in restaurant_in.menu]
    ...
    async def update_restaurant(restaurant_id: int, restaurant_in: RestaurantUpdate) -> dict:
        update_data = restaurant_in.model_dump(exclude_unset=True, exclude_none=True)
        if not update_data:
        ...
        if 'menu' in update_data:
            update_data['menu'] = [item.model_dump() for item in restaurant_in.menu]
