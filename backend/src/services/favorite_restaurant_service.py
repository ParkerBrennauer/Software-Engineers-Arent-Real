...
class FavoriteRestaurantService:
    ...
    async def _restaurant_exists(restaurant_id: int) -> bool:
        ...
        restaurant_records = (
            restaurants.values() if isinstance(restaurants, dict) else restaurants
        )
    ...
    async def add_favorite_restaurant(
        ...
        updated_customer = await CustomerRepo.update_by_id(
            customer_id,
            {"favorite_restaurants": updated_favorites},
        )
        if not updated_customer:
            raise ValueError("Customer not found")

        return FavoriteRestaurantMutationResponse(
            customer_id=customer_id,
            restaurant_id=restaurant_id,
            favorite_restaurants=FavoriteRestaurantService._coerce_favorites(
                updated_customer
            ),
