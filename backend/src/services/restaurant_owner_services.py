...
class RestaurantOwnerService:
    ...
    @staticmethod
    async def switch_venue(username: str, restaurant_id: int) -> dict:
        ...
        if user_role not in [
            ...
            raise ValueError("User does not have permission to manage restaurants")
        ...
        if isinstance(restaurants, dict):
            ...
            found = any(
                (
                    isinstance(r, dict)
                    and (
                        r.get("restaurant_id") == restaurant_id
                        or r.get("id") == restaurant_id
                    )
                )
        ...
        updated_user = await UserRepo.update_by_username(
            username, {"restaurant_id": restaurant_id}
        )
        if not updated_user:
            raise ValueError("User not found")
        return UserInternal.model_validate(updated_user)
    ...
    @staticmethod
    def _append_paid_restaurant_order(
        order_data: dict, restaurant_key: str, bucket: list
    ) -> None:
    ...
    async def get_restaurant_orders(restaurant_id: int, username: str) -> list:
        restaurant_key = await RestaurantOwnerService._assert_staff_or_owner(
            restaurant_id, username
        )
        all_orders = await OrderRepo.get_all_orders()
        restaurant_orders = []
        for order_data in all_orders.values():
            RestaurantOwnerService._append_paid_restaurant_order(
                order_data, restaurant_key, restaurant_orders
            )
