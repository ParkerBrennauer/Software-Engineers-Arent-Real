...
from src.schemas.review_schema import DeleteResponse, ReportCreate, ReportResponse, ReviewCreate, ReviewEdit, ReviewEditResponse, ReviewResponse
...
async def test_review_order_success(monkeypatch):
    ...
    async def fake_submit_review(_order_id: str, _payload: ReviewCreate):
        return ReviewResponse(order_id='1d8e87M', restaurant_id=16, review_text='Great food, fast delivery!')
    monkeypatch.setattr(RatingService, 'submit_review', fake_submit_review)
    transport = httpx.ASGITransport(app=app)
    async with httpx.AsyncClient(transport=transport, base_url='http://testserver') as client:
        response = await client.post('/orders/1d8e87M/review', json={'review_text': 'Great food, fast delivery!'})
    assert response.status_code == 200
    assert response.json() == {'order_id': '1d8e87M', 'restaurant_id': 16, 'review_text': 'Great food, fast delivery!'}
...
async def test_review_nonexistent_order(monkeypatch):
    ...
    transport = httpx.ASGITransport(app=app)
    async with httpx.AsyncClient(transport=transport, base_url='http://testserver') as client:
        response = await client.post('/orders/NONEXISTENT_ORDER/review', json={'review_text': 'Great food!'})
...
async def test_review_order_duplicate(monkeypatch):
    ...
    transport = httpx.ASGITransport(app=app)
    async with httpx.AsyncClient(transport=transport, base_url='http://testserver') as client:
        response = await client.post('/orders/1d8e87M/review', json={'review_text': 'Trying again'})
...
async def test_review_restaurant_not_found(monkeypatch):
    ...
    transport = httpx.ASGITransport(app=app)
    async with httpx.AsyncClient(transport=transport, base_url='http://testserver') as client:
        response = await client.post('/orders/1d8e87M/review', json={'review_text': 'Great food!'})
    assert response.status_code == 404
...
async def test_edit_review_success(monkeypatch):
    ...
    async def fake_edit_order_review(_order_id: str, _payload: ReviewEdit):
        return ReviewEditResponse(order_id='1d8e87M', submitted_stars=5, review_text='Updated review')
    monkeypatch.setattr(RatingService, 'edit_order_review', fake_edit_order_review)
    transport = httpx.ASGITransport(app=app)
    async with httpx.AsyncClient(transport=transport, base_url='http://testserver') as client:
        response = await client.put('/orders/1d8e87M/review', json={'stars': 5, 'review_text': 'Updated review'})
    assert response.status_code == 200
    assert response.json() == {'order_id': '1d8e87M', 'submitted_stars': 5, 'review_text': 'Updated review'}
...
async def test_delete_review_success(monkeypatch):
    ...
    async def fake_delete_order_review(_order_id: str):
        return DeleteResponse(order_id='1d8e87M', message='Review and rating deleted successfully')
    ...
    assert response.status_code == 200
    assert response.json() == {'order_id': '1d8e87M', 'message': 'Review and rating deleted successfully'}
...
async def test_feedback_prompt_success(monkeypatch):
    ...
    async def fake_check_feedback_prompt(_order_id: str):
        return {'order_id': '1d8e87M', 'prompt_feedback': True, 'message': 'How was your order? Leave a rating and review!'}
    monkeypatch.setattr(RatingService, 'check_feedback_prompt', fake_check_feedback_prompt)
    transport = httpx.ASGITransport(app=app)
    async with httpx.AsyncClient(transport=transport, base_url='http://testserver') as client:
        response = await client.get('/orders/1d8e87M/feedback-prompt')
    assert response.status_code == 200
    assert response.json() == {'order_id': '1d8e87M', 'prompt_feedback': True, 'message': 'How was your order? Leave a rating and review!'}
...
async def test_filter_reviews_success(monkeypatch):
    ...
    async def fake_get_filtered_reviews(_restaurant_id: int, stars: int | None=None):
        ...
        return {'restaurant_id': 16, 'stars_filter': None, 'total_reviews': 2, 'reviews': [{'order_id': 'abc123', 'submitted_stars': 5, 'review_text': 'Amazing!'}, {'order_id': 'def456', 'submitted_stars': 3, 'review_text': 'Okay'}]}
    monkeypatch.setattr(RatingService, 'get_filtered_reviews', fake_get_filtered_reviews)
    transport = httpx.ASGITransport(app=app)
    async with httpx.AsyncClient(transport=transport, base_url='http://testserver') as client:
...
async def test_filter_reviews_by_stars(monkeypatch):
    ...
    async def fake_get_filtered_reviews(_restaurant_id: int, stars: int | None=None):
        assert stars == 5
        return {'restaurant_id': 16, 'stars_filter': 5, 'total_reviews': 1, 'reviews': [{'order_id': 'abc123', 'submitted_stars': 5, 'review_text': 'Amazing!'}]}
    monkeypatch.setattr(RatingService, 'get_filtered_reviews', fake_get_filtered_reviews)
    transport = httpx.ASGITransport(app=app)
    async with httpx.AsyncClient(transport=transport, base_url='http://testserver') as client:
...
async def test_report_review_success(monkeypatch):
    ...
    async def fake_submit_report(_order_id: str, _payload: ReportCreate):
        return ReportResponse(report_id=1, order_id='1d8e87M', reason='spam', description=None, message='Report submitted successfully')
    ...
    assert response.status_code == 200
    assert response.json() == {'report_id': 1, 'order_id': '1d8e87M', 'reason': 'spam', 'description': None, 'message': 'Report submitted successfully'}
