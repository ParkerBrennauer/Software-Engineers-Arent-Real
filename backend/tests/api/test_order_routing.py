...
def test_get_order_status():
    ...
    with patch(
        "src.api.routers.order_router.UserService.get_current_user",
        return_value="customer1",
    ):
        with patch(
            "src.api.routers.order_router.OrderRepo.get_by_id",
            new=AsyncMock(return_value={"id": 1, "customer": "customer1"}),
        ):
            with patch(
