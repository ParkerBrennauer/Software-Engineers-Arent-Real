...
async def test_assign_staff_success(client, monkeypatch):
    ...
    owner_user = {'id': 1, 'username': 'owner1', 'email': 'owner@example.com', 'name': 'Owner', 'role': 'owner', 'restaurant_id': 10, 'is_active': True}
    target_user = {'id': 2, 'username': 'user123', 'email': 'user@example.com', 'name': 'User', 'role': 'customer', 'hashed_password': 'hashed', 'is_active': True}
    ...
    async def fake_update_by_username(username: str, updates: dict):
        ...
        return None
    monkeypatch.setattr('src.services.restaurant_owner_services.UserRepo.get_by_username', fake_get_by_username)
    monkeypatch.setattr('src.services.restaurant_owner_services.UserRepo.update_by_username', fake_update_by_username)
...
async def test_assign_staff_not_owner(client, monkeypatch):
    payload = {'staff_username': 'user123'}
    customer_user = {'id': 1, 'username': 'customer1', 'email': 'customer@example.com', 'name': 'Customer', 'role': 'customer', 'is_active': True}
...
async def test_assign_staff_cannot_assign_owner(client, monkeypatch):
    ...
    owner_user = {'id': 1, 'username': 'owner1', 'email': 'owner@example.com', 'name': 'Owner', 'role': 'owner', 'restaurant_id': 10, 'is_active': True}
    another_owner = {'id': 2, 'username': 'another_owner', 'email': 'owner2@example.com', 'name': 'Owner 2', 'role': 'owner', 'restaurant_id': 5, 'is_active': True}
...
async def test_assign_staff_already_staff(client, monkeypatch):
    ...
    owner_user = {'id': 1, 'username': 'owner1', 'email': 'owner@example.com', 'name': 'Owner', 'role': 'owner', 'restaurant_id': 10, 'is_active': True}
    staff_user = {'id': 2, 'username': 'staff1', 'email': 'staff@example.com', 'name': 'Staff', 'role': 'staff', 'restaurant_id': 10, 'hashed_password': 'hashed', 'is_active': True}
    ...
    async def fake_update_by_username(_username: str, _updates: dict):
        raise AssertionError('update_by_username should not be called for existing staff')
    monkeypatch.setattr('src.services.restaurant_owner_services.UserRepo.get_by_username', fake_get_by_username)
    monkeypatch.setattr('src.services.restaurant_owner_services.UserRepo.update_by_username', fake_update_by_username)
    response = client.post('/restaurant_administration/owner1/staff', json=payload)
    assert response.status_code == status.HTTP_200_OK
...
async def test_assign_staff_owner_no_restaurant(client, monkeypatch):
    payload = {'staff_username': 'user123'}
    owner_user = {'id': 1, 'username': 'owner1', 'email': 'owner@example.com', 'name': 'Owner', 'role': 'owner', 'restaurant_id': None, 'is_active': True}
