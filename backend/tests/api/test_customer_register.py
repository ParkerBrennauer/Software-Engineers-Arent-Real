...
async def test_register_customer_success(client, monkeypatch):
    ...
    customer_payload = {
        ...
        "payment_details": "4532015112830366",
    }
    ...
    monkeypatch.setattr(
        "src.services.user_service.UserRepo.get_by_username", fake_get_by_username
    )
    monkeypatch.setattr("src.services.user_service.UserRepo.save_user", fake_save_user)
    monkeypatch.setattr(
        "src.services.user_service.UserService.get_password_hash",
