...
async def test_reset_password_success(monkeypatch):
    ...
    user_data = {'id': 1, 'username': 'user1', 'two_factor_code': '123456', 'two_factor_expires_at': expires, 'hashed_password': 'old_hash'}
    captured = {}
...
async def test_reset_password_invalid_code(monkeypatch):
    expires = (datetime.now(timezone.utc) + timedelta(minutes=5)).isoformat()
    user_data = {'id': 1, 'username': 'user1', 'two_factor_code': '123456', 'two_factor_expires_at': expires}
...
async def test_reset_password_no_code_generated(monkeypatch):
    user_data = {'id': 1, 'username': 'user1', 'two_factor_code': None, 'two_factor_expires_at': None}
