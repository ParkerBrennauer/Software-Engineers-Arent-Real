...
async def test_create_customer_success():
    customer_in = CustomerRegister(email='customer@example.com', name='John Customer', role=UserRole.CUSTOMER, username='johncust1', password='secret123', payment_type='credit card', payment_details='1234567812345678')
    captured = {}
...
async def test_create_customer_invalid_card_non_digits():
    with pytest.raises(ValidationError) as exc_info:
        CustomerRegister(email='customer@example.com', name='John Customer', role=UserRole.CUSTOMER, username='johncust2', password='secret123', payment_type='credit card', payment_details='abcd1234efgh5678')
    assert 'Card number must contain only digits' in str(exc_info.value)

@pytest.mark.asyncio
async def test_create_customer_invalid_card_length():
    with pytest.raises(ValidationError) as exc_info:
        CustomerRegister(email='customer@example.com', name='John Customer', role=UserRole.CUSTOMER, username='johncust3', password='secret123', payment_type='credit card', payment_details='123456')
    assert 'Card number must be 15 or 16 digits' in str(exc_info.value)

@pytest.mark.asyncio
async def test_create_customer_invalid_payment_type():
    with pytest.raises(ValidationError) as exc_info:
        CustomerRegister(email='customer@example.com', name='John Customer', role=UserRole.CUSTOMER, username='johncust4', password='secret123', payment_type='paypal', payment_details='1234567812345678')
    assert 'Payment type must be either credit card or debit card' in str(exc_info.value)

@pytest.mark.asyncio
async def test_create_customer_duplicate_username_raises():
    customer_in = CustomerRegister(email='customer@example.com', name='John Customer', role=UserRole.CUSTOMER, username='taken', password='secret123', payment_type='credit card', payment_details='1234567812345678')
...
async def test_create_customer_debit_card():
    customer_in = CustomerRegister(email='customer@example.com', name='Jane Customer', role=UserRole.CUSTOMER, username='janecust1', password='secret123', payment_type='debit card', payment_details='9876543210123456')
    captured = {}
...
async def test_create_customer_15_digit_card():
    customer_in = CustomerRegister(email='customer@example.com', name='Alex Customer', role=UserRole.CUSTOMER, username='alexcust1', password='secret123', payment_type='credit card', payment_details='123456789012345')
    captured = {}
