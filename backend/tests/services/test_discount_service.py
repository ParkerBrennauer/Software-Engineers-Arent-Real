...
async def test_apply_discount_success(mock_find_savings):
    mock_find_savings.return_value = 0.8
...
async def test_create_discount_success(mock_save_code, mock_check_real):
    ...
    mock_save_code.return_value = {'SAVE10': {'restaurant_id': 1, 'discount_rate': 0.1}}
    result = await DiscountServices.createDiscount(0.1, 'SAVE10', 1)
    assert result == 'valid code, enjoy saving'
    mock_save_code.assert_awaited_once_with({'SAVE10': {'restaurant_id': 1, 'discount_rate': 0.1}})
