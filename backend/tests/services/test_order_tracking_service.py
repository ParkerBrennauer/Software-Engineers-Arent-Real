...
async def test_get_tracking_info_generates_metrics_for_restaurant_stage(monkeypatch):
    order = {'restaurant': 'Restaurant_16', 'customer': 'customer_1', 'driver': None, 'order_status': OrderStatus.PREPARING.value, 'distance': 0.0, 'time': 0}
    saved_updates = {}
    ...
    monkeypatch.setattr(get_order_tracking_service_module().random, 'uniform', lambda _a, _b: 7.25)
    monkeypatch.setattr(get_order_tracking_service_module().random, 'randint', lambda _a, _b: 18)
...
async def test_refresh_tracking_updates_in_transit_metrics(monkeypatch):
    current_order = {'restaurant': 'Restaurant_4', 'customer': 'customer_1', 'driver': 'driver_9', 'order_status': OrderStatus.OUT_FOR_DELIVERY.value, 'distance': 6.5, 'time': 20}
    ...
    monkeypatch.setattr(OrderRepo, 'get_by_id', fake_get_by_id)
    monkeypatch.setattr(OrderRepo, 'update_order', fake_update_order)
    monkeypatch.setattr(get_order_tracking_service_module().random, 'uniform', lambda _a, _b: 1.25)
    monkeypatch.setattr(get_order_tracking_service_module().random, 'randint', lambda _a, _b: 4)
...
async def test_update_tracking_status_marks_order_delivered(monkeypatch):
    current_order = {'restaurant': 'Restaurant_7', 'customer': 'customer_4', 'driver': 'driver_1', 'order_status': OrderStatus.OUT_FOR_DELIVERY.value, 'distance': 3.6, 'time': 12}
    ...
    monkeypatch.setattr(OrderRepo, 'get_by_id', fake_get_by_id)
    monkeypatch.setattr(OrderRepo, 'update_order', fake_update_order)
    tracking = await OrderTrackingService.update_tracking_status('8', OrderTrackingStatusUpdate(order_status=OrderStatus.DELIVERED))
...
async def test_get_tracking_info_cancelled_order_is_not_marked_delivered(monkeypatch):
    order = {'restaurant': 'Restaurant_2', 'customer': 'customer_6', 'driver': None, 'order_status': OrderStatus.CANCELLED.value, 'distance': 4.5, 'time': 19}
...
async def test_get_tracking_info_includes_delivery_instructions(monkeypatch):
    order = {'restaurant': 'Restaurant_2', 'customer': 'customer_6', 'driver': None, 'order_status': OrderStatus.CONFIRMED.value, 'distance': 4.5, 'time': 19, 'delivery_instructions': 'Leave at front door'}
