...
async def test_single_user_login_enforcement(monkeypatch):
    user_data1 = {'id': 1, 'username': 'user1', 'email': 'user1@example.com', 'name': 'User One', 'role': 'customer', 'hashed_password': '$2b$12$abcdefghijklmnopqrstuvwxyz', 'is_active': True, 'requires_2fa': False}
    user_data2 = {'id': 2, 'username': 'user2', 'email': 'user2@example.com', 'name': 'User Two', 'role': 'customer', 'hashed_password': '$2b$12$abcdefghijklmnopqrstuvwxyz', 'is_active': True, 'requires_2fa': False}
    ...
    with pytest.raises(ValueError, match='Another user \\(user1\\) is already logged in.'):
