...
async def test_tip_driver_success(mock_order_service):
    mock_order_service.get_order_status = AsyncMock(return_value={'id': 1, 'tip_amount': 10, 'driver': 'driver_1', 'tip_paid': False})
...
async def test_tip_driver_no_tip(mock_order_service):
    mock_order_service.get_order_status = AsyncMock(return_value={'tip_amount': 0, 'driver': 'driver_1'})
...
async def test_tip_driver_already_paid(mock_order_service):
    mock_order_service.get_order_status = AsyncMock(return_value={'tip_amount': 10, 'driver': 'driver_1', 'tip_paid': True})
    mock_order_service.update_order = AsyncMock()
...
async def test_tip_driver_no_driver(mock_order_service):
    mock_order_service.get_order_status = AsyncMock(return_value={'tip_amount': 10, 'driver': None, 'tip_paid': False})
    with pytest.raises(ValueError):
        await DriverService.tip_driver(1)
