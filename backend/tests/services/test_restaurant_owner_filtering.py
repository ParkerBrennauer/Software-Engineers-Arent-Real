...
async def test_filter_by_status_owner(monkeypatch):
    ...
    result = await RestaurantOwnerService.get_restaurant_orders_by_status(
        10, "owner1", OrderStatus.CONFIRMED.value
    )
...
async def test_filter_by_status_denied(monkeypatch):
    ...
    with pytest.raises(ValueError, match="does not have permission"):
        await RestaurantOwnerService.get_restaurant_orders_by_status(
            20, "owner1", OrderStatus.CONFIRMED.value
        )
...
async def test_filter_by_date_range(monkeypatch):
    ...
    result = await RestaurantOwnerService.get_restaurant_orders_by_date_range(
        10, "owner1", 100, 200
    )
    assert len(result) == 1
...
async def test_filter_by_status_and_date(monkeypatch):
    ...
    result = await RestaurantOwnerService.get_restaurant_orders_by_status_and_date(
        10, "owner1", OrderStatus.CONFIRMED.value, 100, 200
    )
    assert len(result) == 1
...
async def test_filter_by_status_staff_access(monkeypatch):
    ...
    result = await RestaurantOwnerService.get_restaurant_orders_by_status(
        10, "staff1", OrderStatus.CONFIRMED.value
    )
    assert len(result) == 1
...
async def test_filter_user_not_found(monkeypatch):
    ...
    with pytest.raises(ValueError, match="User not found"):
        await RestaurantOwnerService.get_restaurant_orders_by_status(
            10, "unknown", OrderStatus.CONFIRMED.value
        )
