...
async def test_create_order_with_no_existing_orders(mock_save, mock_get_largest):
    ...
    mock_save.return_value = {'id': 1, 'items': [{'id': 1, 'price': 10.0}], 'cost': 11.3, 'order_status': 'payment pending', 'payment_status': 'pending', 'locked': False}
    order_create = OrderCreate(items=[{'id': 1, 'price': 10.0}], cost=10.0, restaurant='Test', customer='test_user', time=30, cuisine='Italian', distance=2.0)
...
async def test_lock_already_locked_order(mock_update, mock_get):
    mock_get.return_value = {'id': 1, 'locked': True, 'order_status': 'pending', 'payment_status': 'unpaid', 'items': [], 'cost': 0}
...
async def test_get_items_by_restaurant_id(mock_get):
    mock_get.return_value = [{'item_name': 'Burger', 'price': 9.99}, {'item_name': 'Pizza', 'price': 12.99}]
...
async def test_create_item_success(mock_save, mock_get):
    ...
    item_create = ItemCreate(item_name='Burger', restaurant_id=1, price=9.99, description='Classic burger', cost=9.99, cuisine='American')
...
async def test_edit_review_success(mock_update, mock_get):
    mock_get.return_value = {'id': 'order_1', 'submitted_stars': 4, 'review_text': 'Good'}
    mock_update.return_value = {'id': 'order_1', 'submitted_stars': 5, 'review_text': 'Excellent!'}
